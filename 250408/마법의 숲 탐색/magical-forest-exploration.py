...
def get_rowmax(forest, r, c, d):   #DFS
    r, c = r - 2, c
    row_max = max(r + gol[d][0], r + 1)  # 본인 위치에서 갈 수 있는 최대
    ...
    myque = deque()
    myque.append((r + gol[d][0], c+ gol[d][1]))  ## 출구위치에서 시작
    while myque:  #idnex R, C로 해도 됨
        start_r, start_c = myque.popleft()
        for dr, dc in find_max:
            nex_r, nex_c = start_r + dr, start_c + dc
            if 0<nex_r<=R-3 and 0<nex_c<=C:
                if visited[nex_r][nex_c] == False and map_f[nex_r][nex_c] !=0: # 갈수 있는 곳
                    visited[nex_r][nex_c] = True
                    row_max = max(row_max, nex_r)
                    myque.append((nex_r, nex_c)) #다음 위치 탐색
...
def find_forest(forest, g_d, ci, answer): #숲이랑 골렘의 출구방향
    ...
    while True:  #남쪽으로 계속 하강 먼저
        if r == R - 2:  # 중심위치 최대면
            ...
            answer += get_rowmax(forest, r, c, g_d)
...
def main():
    global forest, answer


    for c_i, di in com:
        forest, answer = find_forest(forest, di, c_i, answer) # 숲 탐색
