...
def moveOrgan(p, s): 
    ...
    for i in range(len(s)):
        ...
        if sd>0 and p[id]:
            ...
            lc = maxc - minc # 열길이
            placed = False
            for c in range(N): # 0행부터
                for r in range(N): # 1열까지
...
def saveResult(b):
    ...
    while q:
        ...
        for dr, dc in d:
            nr, nc = r + dr, c + dc
            if (0<=nr<N and 0<=nc<N) and visited[nr][nc] == False: # 갈수있으면
                if curid !=0 and b[nr][nc] !=0:  # 0이 아니면 
                    if curid != b[nr][nc]: # 방문가능, 경계면 확인 
                        if ((curid, b[nr][nc]) not in ans) and  ((b[nr][nc], curid) not in ans):
                            ans.append((curid, b[nr][nc]))
                    visited[nr][nc] = True
                    q.append((nr, nc))
