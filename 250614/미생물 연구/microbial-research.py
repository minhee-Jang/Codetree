...
def moveMicro(prior, loc, t):
    ...
    for i in range(len(prior)):
        ...
        if not alive[prior[i][1]]: continue
        minr, minc, maxr, maxc = minmax(prior[i][2]) # 최대최소
        dr, dc = maxr - minr, maxc - minc

        possible = False
        for c in range(N): # c가 작을수록
             for r in range(N): # r이 작을수록
                if in_range(r+dr, c+dc): # 자르기 가능
                    # 검사하고 붙이기  ###########
                    br, bc = r - minr, c - minc
                    for a, b in loc[prior[i][1]][2]:
                        tr, tc = a +br, b +bc
                        if newEx[tr][tc]!=0:
                            possible = False
                            break
                    ...
                    if possible:
                        ...
                        for a, b in loc[prior[i][1]][2]:
                            ...
                            newEx[nbr][nbc] = prior[i][1]
                            newR.append((nbr, nbc))
                        loc[prior[i][1]][2] = newR # 좌표 교체
                        break
             if possible:
                break
